...
async def Start_Match_Logic(key, iv, region):
    # OB52 Match Start Protobuf Payload (Real Signal)
    # 0801 = Match Type, 12020801 = Start Trigger/Version
    start_payload = "080112020801" 
    
    # पेलोड को एनक्रिप्ट करना (xC4 लाइब्रेरी की मदद से)
    encrypted_payload = await EnC_PacKeT(start_payload, key, iv)
    
    # पैकेट की लम्बाई (Length) निकालना
    # zfill(2) यह सुनिश्चित करता है कि लेंथ हमेशा 2 डिजिट में रहे (जैसे 0a, 0f)
    payload_len = hex(len(encrypted_payload) // 2)[2:].zfill(2)
    
    # 0112: मैच स्टार्ट की सर्विस आईडी
    # 000000: स्टैंडर्ड हेडर पैडिंग
    return f"0112000000{payload_len}{encrypted_payload}"
...
async def perform_emote(team_code: str, uids: list, emote_id: int, start_game: bool = False):
    ...
    try:
        ...
        # 🚀 3. REPEATED START (Every 15s)
        if start_game:
            print(">>> Auto-Tap Start Active (15s delay)")
            for i in range(10): 
                ST = await Start_Match_Logic(key, iv, region)
                await SEndPacKeT(None, online_writer, 'OnLine', ST)
...
async def perform_auto_leader(target_uid: int, emote_id: int):
    global key, iv, region, online_writer, BOT_UID
    if online_writer is None: raise Exception("Bot Offline")
    try:
        # स्क्वाड बनाओ
        ...
        # इनवाइट भेजो
        INV = await SEnd_InV(5, target_uid, key, iv, region) 
        await SEndPacKeT(None, online_writer, 'OnLine', INV)
        print(f">>> Invite sent to {target_uid}, waiting 10s...")
        await asyncio.sleep(10) 
        # इमोट और 15 सेकंड वाला स्टार्ट
        ...
        for i in range(5):
            ...
            await SEndPacKeT(None, online_writer, 'OnLine', ST)
            await asyncio.sleep(15)
        await SEndPacKeT(None, online_writer, 'OnLine', await ExiT(None, key, iv))
    except Exception as e: print(f"Leader Error: {e}")

@app.route('/invite')
def invite_and_start():
    global loop
    uid = request.args.get('uid')
    emote = request.args.get('emote_id', '0')

    if not uid:
        return jsonify({"status": "error", "message": "UID is required"})

    try:
        # बैकग्राउंड में प्रोसेस शुरू करना
        asyncio.run_coroutine_threadsafe(
            perform_auto_leader(int(uid), int(emote)), loop
        )
        return jsonify({
            "status": "success", 
            "message": f"Bot is creating squad and inviting {uid}. Please stay in Lobby!"
        })
    except Exception as e:
...
def join_team():
    ...
    should_start = request.args.get('start', 'false').lower() == 'true'
    uids = [request.args.get(f'uid{i}') for i in range(1, 7) if request.args.get(f'uid{i}')]

    if not team_code or not emote_id_str or not uids:
        return jsonify({"status": "error", "message": "Missing params (tc, emote_id, uid1)"})

    try:
        asyncio.run_coroutine_threadsafe(
            perform_emote(team_code, uids, int(emote_id_str), should_start), loop
        )
        return jsonify({"status": "success", "message": "Emote/Start sequence triggered via TeamCode"})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)})
...
async def MaiiiinE():
    ...
    # BOT LOGIN UID
    BOT_UID = int('14695547984')  # <-- FIXED BOT UID

    Uid, Pw = '4501634130', '7A4D844730CF4E20B08811B98A45A6A35A4F97F49C3D8EFCE20AEF673A9DD6E8'
